...
std::vector<cv::Vec3b> find_dominant_colors(cv::Mat img, int count) {
  ...
  //  cv::Mat dom = get_dominant_palette(colors);
    ...
    cv::imwrite("/home/shaggy/line_auv/classification.png", viewable);
    cv::imwrite("/home/shaggy/line_auv/quantized.png", quantized);
...
int main(int argc, char* argv[]) {
    ...
    VideoCapture cap("/home/shaggy/line_auv/GOPR0036.MP4");
    ...
    namedWindow("MyVieo",CV_WINDOW_NORMAL);
    ...
    namedWindow("AB",CV_WINDOW_NORMAL);

    Mat iFrame;

    bool bSuccess = cap.read(iFrame);
    int a,b,c;
  ...
  //      if(argc<3) {
   //     printf("Usage: %s <image> <count>\n", argv[0]);
    ...
    vector<Mat> Split;
    std::vector<cv::Vec3b> colors = find_dominant_colors(image, count);

    Mat Qimg = imread("/home/shaggy/line_auv/quantized.png", CV_LOAD_IMAGE_COLOR);

    a = Qimg.at<cv::Vec3b>(0,0)[0];
    b = Qimg.at<cv::Vec3b>(0,0)[1];
    c = Qimg.at<cv::Vec3b>(0,0)[2];
    ...
    while(1)
    {
        ...
        int x,y;
        Mat imgH = frame; //Scalar(-100,-100,-50); //+ Scalar(50,50,50);
        cvtColor(imgH,img_Lab,CV_BGR2Lab);
        imshow("LAB",img_Lab);
        split(img_Lab,Split);
        imshow("L",Split[0]);
        imshow("A",Split[1]);
        imshow("B",Split[2]);
        Split[0]=Mat::zeros(Split[1].rows,Split[1].cols,CV_8UC1);
        merge(Split,img_AB);
        imshow("AB",img_AB);

        for(y=0; y<imgH.rows; y++){
            for(x=0; x<imgH.cols; x++){
                if(imgH.at<cv::Vec3b>(y,x)[1] >= b){
                    imgH.at<cv::Vec3b>(y,x)[0] = 255;
                    imgH.at<cv::Vec3b>(y,x)[1] = 255;
                    imgH.at<cv::Vec3b>(y,x)[2] = 255;
                }
                else{
                    imgH.at<cv::Vec3b>(y,x)[0] = 0;
                    imgH.at<cv::Vec3b>(y,x)[1] = 0;
                    imgH.at<cv::Vec3b>(y,x)[2] = 0;
                }
                if(imgH.at<cv::Vec3b>(y,x)[1] >= b && imgH.at<cv::Vec3b>(y,x)[0] <= 20 && imgH.at<cv::Vec3b>(y,x)[2] <= 20/*&& imgH.at<cv::Vec3b>(y,x)[2] <= 40*/){
                    imgH.at<cv::Vec3b>(y,x)[0] = 0;
                    imgH.at<cv::Vec3b>(y,x)[1] = 0;
                    imgH.at<cv::Vec3b>(y,x)[2] = 0;
                }
